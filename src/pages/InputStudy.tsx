...
export default function InputStudy() {
  ...
  const isMasteredMode = location.includes("?mode=mastered");
  ...
  const {
    ...
    groupId,
    mode: isMasteredMode ? "mastered" : "normal"
  });
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl font-heading font-bold text-slate-900 dark:text-white">
            {isMasteredMode ? "Ôn tập từ đã thuộc: " : ""}
            {groupDetails?.name}
          </h2>
        </div>
      ...
      <div className="mb-4 max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-slate-600 dark:text-slate-400">
            {isMasteredMode ? "Ôn tập từ đã thuộc: " : ""}
            Từ {completedWords + 1}/{studyWords.length}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center">
        {studyWords[currentIndex] && (
          <WordCardInput
            ...
            isMasteredReview={isMasteredMode}
          />
        )}
      </div>
